...
public class SWItemTurret extends SWTurret {
    public ObjectMap<Item, BulletType> ammoTypes = new OrderedMap<>();

    public SWItemTurret(String name){
        super(name);
        hasItems = true;
    }
    ...
    public void setStats(){
        ...
        stats.add(Stat.ammo, SWStat.ammo(ammoTypes, false, false, bulletsChangeTargeting, name));
    ...
    public void setBars(){
        ...
        addBar("ammo", (SWItemTurretBuild entity) ->
            new Bar(
                "stat.ammo",
                Pal.ammo,
    ...
    public void init(){
        consume(new ConsumeItemFilter(i -> ammoTypes.containsKey(i)){
            ...
            public void build(Building build, Table table){
                ...
                () -> build instanceof SWItemTurretBuild it && !it.ammo.isEmpty() && ((ItemEntry)it.ammo.peek()).item == item)));
            ...
            public float efficiency(Building build){
                ...
                return build instanceof SWItemTurretBuild it && it.ammo.size > 0 && (it.ammo.peek().amount >= ammoPerShot || it.cheating()) ? 1f : 0f;
            }
        ...
        if(targetGround){
            ammoTypes.each((item, type) -> placeOverlapRange = Math.max(placeOverlapRange, (type.collidesGround || !bulletsChangeTargeting) ? range + type.rangeChange + placeOverlapMargin : 0f));
        }

        super.init();
    }

    public class SWItemTurretBuild extends SWTurretBuild{
