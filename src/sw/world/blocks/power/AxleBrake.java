	...
	public class AxleBrakeBuild extends AxleBlockBuild {
		...
		public void buildConfiguration(Table cont) {
			cont.table(Styles.black6, table -> {
				buildFloatSlider(table, "ui.sw-max-speed", value -> configure(new float[]{value / 10f, torqueTarget, torqueGradient}), () -> speedTarget * 10f);
				if (strength < 0) {
					table.image(Tex.whiteui).color(Color.gray).padTop(10f).padBottom(10f).height(4f).growX().row();
					buildFloatSlider(table, "ui.sw-brake-strength", value -> configure(new float[]{speedTarget, value / 600f, torqueGradient}), () -> torqueTarget * 600f);
				}
				if (speedGradient < 0) {
					table.image(Tex.whiteui).color(Color.gray).padTop(10f).padBottom(10f).height(4f).growX().row();
					buildFloatSlider(table, "ui.sw-brake-gradient", value -> configure(new float[]{speedTarget, torqueTarget, value / 10f}), () -> torqueGradient * 10f);
				}
			}).margin(10f);
		}
